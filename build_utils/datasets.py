...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self,
                 ...
                 single_cls=False, pad=0.0, rank=-1, skimage=None):  # rank参数只有在多gpu训练时才有用
        ...
        if self.rect:
            ...
            irect = ar.argsort()  # argsort函数返回的是数组值从小到大的索引值
            # 根据排序后的顺序重新设置图像文件名顺序、标签文件名顺序、shape顺序以及高宽比顺序
            ...
            for i in range(nb):
                # numpy中的一个小技巧：
                # bi = np.array([0, 0, 1, 1, 2, 2])
                # print(bi == 1)
                # [False False  True  True False False]
                # 结果会得到一个布尔值列表，反应相等值在数组中的情况
                ...
                if maxi < 1:  # 即图片身子宽
                    shapes[i] = [maxi, 1]  # 图像宽度一定是img_size指定的大小
                    # 其中高宽比最大的图像一般能够完整的塞入相框中，而其他需要一定的填充
                # 如果高/宽大于1(w < h)，将h设置为img_size
                elif mini > 1:  # 即图片身子高瘦
                    shapes[i] = [1, 1 / mini]  # 图像的高度一定是img_size指定的大小
                    # 其中高宽比最小的图像一般能够完整的塞入相框中，而其他需要一定的填充
            # 计算每个batch输入网络的shape值(向上设置为32的整数倍)
            self.batch_shapes = np.ceil(np.array(shapes) * img_size / 32. + pad).astype(np.int32) * 32

        # cache labels 缓存标注信息到内存（/显存？）中
        ...
        if os.path.isfile(np_labels_path):
            ...
            # x存储着训练集各个文件的标注信息，每一个标注信息都以二维矩阵进行记录，诸如：
            # [[          0     0.89649     0.52564    0.053189     0.15047],
            # [          0     0.92527     0.50204    0.044515     0.15499]]
            # 一个行向量表示一个目标的[cls_idx x y w h ]
            if len(x) == n:
        ...
        for i, file in enumerate(pbar):
            ...
            if l.shape[0]:
                ...
                if np.unique(l, axis=0).shape[0] < l.shape[0]:  # duplicate rows
                    nd += 1
                if single_cls:
                    l[:, 0] = 0  # force Kaist into single-class mode

                self.labels[i] = l  # 将当前标注信息真正的缓存到Dataset的labels成员变量中
                nf += 1  # file found

                # Extract object detection boxes for a second stage classifier
                # 将相应的目标从图像中裁剪出来，方便需要的分类网络训练之用，不过我们这里实际上并不需要
        ...
        if cache_images:  # if training 默认不使用
        ...
        detect_corrupted_images = False  # 是否检测图片是否破损
    ...
    def __getitem__(self, index):
        ...
        if self.mosaic:
            ...
            # letterbox，其中self.batch记录了每一个图片是属于哪一个batch的
        ...
        img = np.ascontiguousarray(img)  # 该函数将一个内存不连续存储的数组转换为内存连续存储的数组，使得运行速度更快
...
def load_image(self, index):
    # loads 1 image from Kaist, returns img, original hw, resized hw
    img = self.imgs[index]
...
def load_mosaic(self, index):
    ...
    # Concat/clip labels
    if len(labels4):
        # 四合交叉得到的图像上的所有标注GT boxes的所有信息在维度0上进行拼接,
        # 等到的矩阵大小为: (len(labels4), 1 + 4)
        labels4 = np.concatenate(labels4, 0)
        # 设置上下限防止越界，其中np.clip()是一个截取函数
...
def random_affine(img, targets=(), degrees=10, translate=.1, scale=.1, shear=10, border=0):
    ...
    # Shear
    # 生成错切矩阵，横向错切和纵向错切最大10度
    S = np.eye(3)
    S[0, 1] = math.tan(random.uniform(-shear, shear) * math.pi / 180)  # x shear (deg)
    S[1, 0] = math.tan(random.uniform(-shear, shear) * math.pi / 180)  # y shear (deg)

    # Combined rotation matrix
    # 其中@用于矩阵乘法，而*则是将两个矩阵对应位相乘（此时矩阵大小必须相同）
    # 注意我们的处理过程包括：①先旋转、缩放、平移到4合图像中心；②再将图像平移到4合图像的左上角
    # 1/4图像的中心点位置；③最后将图像进行错切（在横向上以及纵向上）
    M = S @ T @ R  # ORDER IS IMPORTANT HERE!!
    if (border != 0) or (M != np.eye(3)).any():  # image changed
        # 即在图像需要从4合中心平移到左上角1/4图像中心，或者放射变换矩阵不为单位矩阵的时候进行仿射变化
        img = cv2.warpAffine(img, M[:2], dsize=(width, height), flags=cv2.INTER_LINEAR, borderValue=(114, 114, 114))

    # Transform label coordinates
    n = len(targets)
    if n:
        # targets就是这张图片上的所有标注信息组成的矩阵，形式如下：
        # [[class_index, xmin, ymin, xmax, ymax],...]
        # warp points
        xy = np.ones((n * 4, 3))
        # 对targets中的每一行取索引为1、2、3、4、1、4、3、2的元素，然后reshape成一个
        # 大小为[n * 4, 2]的矩阵，每一行两个元素分别记录这这个标注边界框的4个角的顶点坐标。
        # 这样的变换类似于如下的过程：
        # [[2 3 4 5 2 5 4 3]] ->
        # [[2. 3. 1.]
        #  [4. 5. 1.]
        ...
        # [4*n, 3] -> [n, 8]
        # 计算这些标注边界框在进行仿射变换之后的4个顶点的坐标
        xy = (xy @ M.T)[:, :2].reshape(n, 8)

        # create new boxes
        # 对transform后的bbox进行修正(假设变换后的bbox变成了菱形，此时要修正成矩形)
        x = xy[:, [0, 2, 4, 6]]  # [n, 4] 记录x1，x2，x3，x4，其实这个[:,]中的:有for的意味
        y = xy[:, [1, 3, 5, 7]]  # [n, 4] 记录y1，y2，y3，y4
        # 取出x维度1上的最小值、最大值分别作为xmin，xmax，y维度1上的最小值、最大值作为ymin，ymax
        xy = np.concatenate((x.min(axis=1), y.min(1), x.max(1), y.max(1))).reshape(4, n).T  # [n, 4]‘
        # 此时xy记录的是最终经过仿射变换之后的边界框信息
