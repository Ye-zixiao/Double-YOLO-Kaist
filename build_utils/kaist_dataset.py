...
class LoadKaistImagesAndLabels(Dataset):
    def __init__(self,
                 ...
                 pad=0.0):
        # 1、检查数据指定文件是否存在，若存在则读取之并加载到img_files中
        ...
        try:
            ...
            self.img_files = [x for x in f if os.path.splitext(x)[-1].lower() in img_formats]  # 无类型图像文件名列表
            # TODO: 这里只是替换了文件名最后的.jpg为xxx.jpg，但并不是最佳方案，因为有些文件可能不是该类型
            self.visible_img_files = [x.replace(".jpg", "_visible.jpg") for x in self.img_files]  # 可见光图像文件名列表
            self.lwir_img_files = [x.replace(".jpg", "_lwir.jpg") for x in self.img_files]  # 红外光图像文件名列表
        ...
        #    否则所有的图片都会默认放缩到(img_size x img_size)的图像大小。但四合变换和rect变换只能选其一
        if self.rect:
            # TODO: 暂时不要使用rect变换，因为当前版本下的rect变换会使得最终输出的图像的标注信息发生严重的偏移
            s = self.shapes
            aspect_ratio = s[:, 1] / s[:, 0]  # 计算高宽比
            rect_index = aspect_ratio.argsort()  # 根据高宽比对下标进行排序，这对于后续图像文件顺序调整非常有用

            # 根据高宽比，调整对象中各个图像文件名列表的顺序
            ...
            for i in range(batch_num):
                ...
                # 若高宽比不等于1，则调整尺寸使得其中最长边等于img_size，做如此适应
                if maxi < 1:
                    shapes[i] = [maxi, 1]
                elif mini > 1:
                    shapes[i] = [1, 1 / mini]
    ...
    def load_quadra_trans_images(self, index):
        '''
    ...
    def __getitem__(self, index):
        '''
        LoadKaistImagesAndLabels的取数据函数，它根据索引读取指定图像（一张可见光和一张红外光图像）并做预处理。
        1、首先它会根据是否开启4合变换的情况，使用四合交叉旋转仿射畸变来对图像、对应标注信息做处理，否则执行rect变换
        到当前批次要求的尺寸；2、然后根据设置情况执行随机图像增强和随机水平翻转
        :param index:
        :return:
        '''
        hyp = self.hyp
        if self.quadra_trans:
            # 加载图像，对按照相同的随机性对可见光和红外光图像进行4合变换
            v_img, l_img, labels = self.load_quadra_trans_images(index)
            shapes = None
        else:
            # 加载可见光和红外光图像
            ...
            x = self.labels[index]
            if x.size > 0:
                # 由于上述图像经过一定的放缩，所以图像上的标注信息也需要按照同比例进行放缩
...
def random_affine(v_img, l_img, labels=(), degrees=10, translate=0.1, scale=0.1, shear=0.1, border=0):
    ...
    '''
    # 计算随即仿射变换后大的四合图像需要变换到的最终图像尺寸高度和宽度
